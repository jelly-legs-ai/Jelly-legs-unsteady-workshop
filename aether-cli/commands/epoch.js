...
const { getEpochInfo, getSlot, httpPost, DEFAULT_RPC_URL } = require('../sdk');
...
// ---------------------------------------------------------------------------
// HTTP helpers - DEPRECATED: Use SDK functions instead
// httpPost is imported from SDK for backwards compatibility
// ---------------------------------------------------------------------------

function getDefaultRpc() {
  return process.env.AETHER_RPC || DEFAULT_RPC_URL;
}
...
// ---------------------------------------------------------------------------
// Fetch epoch info from RPC - NOW USING SDK
// ---------------------------------------------------------------------------

async function fetchEpochInfo(rpc) {
  // 🎉 Try Aether-native SDK endpoint first - REAL RPC CALL
  try {
...
async function showEpochInfo(opts) {
  ...
  console.log('  ' + C.dim + 'Data source: ' + source + C.reset);
  console.log('');
...
async function epochCommand() {
  ...
  try {
    ...
    if (opts.asJson) {
      console.log(JSON.stringify({ error: err.message, sdk_used: true }, null, 2));
    } else {
      console.log('');
