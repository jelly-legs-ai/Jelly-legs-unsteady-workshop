...
mod tests {
    ...
    fn test_slot_confirmation() {
        ...
        // 2/3 stake = 2000, threshold = 2000/3*2 + 1 = 1334
        // With only 2000 stake, not yet confirmed (need > 2/3)
        // threshold = floor(2000) + 1 = 2001 for strict > 2/3
        // Actually: (3000 * 2) / 3 + 1 = 2001, so 2000 >= 2001 is false
        assert!(!tower.is_slot_confirmed(1, total_stake));

        // Third validator votes
        tower.process_vote(v3, 1, 1000).unwrap();
        
        // Now we have 3000 stake, threshold = 2001, 3000 >= 2001 is true
        assert!(tower.is_slot_confirmed(1, total_stake));
    }
    ...
    fn test_fork_validity() {
        ...
        for i in 2..=35 {
            tower.process_vote(v1, i, 1000).unwrap();
        }

        let root = tower.get_root_slot();
        // With 35 votes (slots 1-35):
        // - Slot 1 has 34 descendant votes (confirmations)
        // - Slot 2 has 33 confirmations  
        // - Slot 3 has 32 confirmations
        // Since confirmation_depth is 32, slots 1, 2, and 3 are confirmed
        // Root is the max confirmed slot = 3
        
        // Valid fork descends from root (if root > 0)
        if root > 0 {
            assert!(tower.is_valid_fork(root + 10, &[root, root + 5, root + 10]));
        }
        
        // Invalid fork doesn't include root (works regardless of root value)
